...
public class OrderQueryRepository {
    ...
    public List<OrderQueryDto> findallByDto_optimization() {
        List<OrderQueryDto> result = findOrders(); // 쿼리 1번쨰
        List<Long> orderIds = result.stream().map(o -> o.getOrderId()) //orderId들만 뽑음
                .collect(Collectors.toList());


        List<OrderItemQueryDto> orderItems = findOrderItems(orderIds); // 쿼리 2번쨰
        Map<Long, List<OrderItemQueryDto>> orderItemMap = orderItems.stream()
                .collect(Collectors.groupingBy(OrderItemQueryDto -> OrderItemQueryDto.getOrderId())); // groupBy로 인해 Map으로 바뀜.
